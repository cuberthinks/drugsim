#!/usr/bin/env python3
"""Compute features and the global scaffold split for the DRD2 dataset.

Reuses drugsim_chem directly for every chemistry operation, identical to
`models/admet/herg_inhibition/prepare_features.py` -- same descriptor
set, same Morgan fingerprint parameters, same scaffold-level
split-group assignment scheme (ADR-009). SPLIT_SALT is
endpoint-specific so this dataset gets its own independent split
assignment.

The one difference from the ADMET scripts: `labels` here is `pki`
(float32, continuous), not a binary int8 label.

Usage:
    python models/psychiatric/drd2_activity/prepare_features.py
"""
...
DATASET_CSV = ROOT / "datasets" / "processed" / "drd2_activity_dataset.csv"
OUTPUT_NPZ = ROOT / "datasets" / "processed" / "drd2_activity_features.npz"
OUTPUT_MANIFEST = ROOT / "datasets" / "processed" / "drd2_activity_features_manifest.json"

SPLIT_SALT = "drd2_activity_v1"
N_SPLIT_GROUPS = 10
...
def main() -> int:
    ...
    labels = df["pki"].to_numpy(dtype=np.float32)
    ...
    manifest = {
        ...
        "split_assignment": {
            ...
            "group_role": {"0-6": "train", "7": "calibration (conformal regression)", "8": "validation", "9": "test"},
