#!/usr/bin/env python3
"""Fetch the complete real CYP2D6 (CHEMBL289) IC50 activity set from ChEMBL.

Mirrors ``models/admet/cyp3a4_inhibition/fetch_chembl_data.py`` exactly
(same retrieval strategy, pagination, manifest shape) -- target and
metadata differ only.

Endpoint choice: `docs/psychiatric-pipeline/data-sources.md`'s CYP2D6
section. Phase 9's own `endpoint-selection.md` rejected CYP2D6 on
sample size, but that rejection used the WRONG ChEMBL target ID
(CHEMBL2035, which is actually the muscarinic acetylcholine receptor
M5, not CYP2D6 -- confirmed live via target_search on 2026-08-30). The
real CYP2D6 target, CHEMBL289 ("Cytochrome P450 2D6," UniProt P10635),
has 8,684 raw IC50 records / 3,349 after nM+pChEMBL filtering
(live-verified 2026-08-30) -- a real, moderate candidate, not
insufficient data.

Usage:
    python models/psychiatric/cyp2d6_activity/fetch_chembl_data.py
"""
...
OUTPUT_CSV = ROOT / "datasets" / "raw" / "chembl_cyp2d6_ic50_raw.csv"
OUTPUT_MANIFEST = ROOT / "datasets" / "raw" / "chembl_cyp2d6_ic50_manifest.json"
...
def main() -> int:
    ...
    manifest = {
        ...
        "url": BASE_URL,
        "target_chembl_id": TARGET_CHEMBL_ID,
        "target_name": "Cytochrome P450 2D6 (CYP2D6)",
        "standard_type": STANDARD_TYPE,
        ...
        "note": (
            ...
            "ChEMBL had for CHEMBL289 on the retrieval date, via full "
