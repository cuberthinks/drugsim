#!/usr/bin/env python3
"""Train the BBB permeability baseline classifier.

Classical models only (Random Forest, Gradient Boosting, Logistic
Regression), same reasoning already applied to every other endpoint in
this repository: this dataset (~1.3k training compounds) is a
small-data regime, no GNN infrastructure exists here, and one is not
mandated by the feature brief.

Predicts a binary BBB-permeability label (1=BBB-permeant,
0=BBB-non-permeant, as published by Martins et al.), not a continuous
value -- BBB has no selectivity-metric requirement demanding a
directly comparable continuous scale the way DRD2/HRH1 did.
...
Usage:
    python models/psychiatric/bbb_permeability/train.py
"""
...
def main() -> int:
    ...
    print(f"train: {x_train.shape[0]} compounds ({y_train.mean():.1%} BBB+)", file=sys.stderr)
    print(f"validation: {x_val.shape[0]} compounds ({y_val.mean():.1%} BBB+)", file=sys.stderr)
    ...
    manifest = {
        ...
        "gnn_benchmark": (
            ...
            "to DRD2/HRH1/CYP2D6/CYP3A4/hERG in this repository."
        ),
