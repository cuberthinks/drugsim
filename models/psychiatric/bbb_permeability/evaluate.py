#!/usr/bin/env python3
"""Evaluate the BBB classifier on the held-out test group -- touched once, here.

Mirrors ``models/psychiatric/cyp2d6_activity/evaluate.py`` exactly --
same binary classification metrics, same split-conformal
prediction-set methodology, same exclude-self-fixed Tanimoto+kNN
applicability domain.

Usage:
    python models/psychiatric/bbb_permeability/evaluate.py
"""
...
FEATURES_NPZ = ROOT / "datasets" / "processed" / "bbb_permeability_features.npz"
...
def _tanimoto_max_similarity(query_fps: np.ndarray, train_fps: np.ndarray, exclude_self: bool = False) -> np.ndarray:
    ...
    drd2_activity/evaluate.py's identical function docstring for the
    self-match bug this guards against.
    """
...
def main() -> int:
    ...
    result = {
        ...
        "label": "binary BBB permeability (1=BBB-permeant, 0=BBB-non-permeant, per Martins et al.)",
        "global_split": {
            **global_metrics,
        ...
        "external_validation": {
            "performed": False,
            "reason": "No independent second-source BBB dataset identified in this pass, same situation as every other new endpoint in this pipeline.",
        },
        "limitations": [
            "BBB permeability here is a binary passive/measured brain-plasma partitioning call, not "
            "CNS efficacy or exposure at a therapeutic dose -- 'BBB-permeant' does not mean "
            "'clinically active in the CNS.' See scientific-foundation.md's BBB-lipophilicity "
            "section: LogP correlates with passive permeability but is neither necessary nor "
            "sufficient (active transport, TPSA/HBD are often better predictors).",
            "Class imbalance (~76% BBB-permeant in the raw source) means ROC-AUC/balanced accuracy "
            "should be read alongside precision/recall/MCC, not alone.",
