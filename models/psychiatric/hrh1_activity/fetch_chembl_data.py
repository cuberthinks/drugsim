#!/usr/bin/env python3
"""Fetch the complete real HRH1 (CHEMBL231) Ki activity set from ChEMBL.

Mirrors `models/psychiatric/drd2_activity/fetch_chembl_data.py` exactly
-- same standalone, paginate-the-whole-population approach, same
raw/manifest output convention. Only the target differs.

Endpoint choice (`docs/psychiatric-pipeline/data-sources.md`): Ki, not
IC50 -- dominant for this target (2,893 Ki vs. 1,334 IC50 records,
live-verified during this feature's data-source audit) and consistent
with DRD2's own choice, so the two targets' predicted values are on the
same scale for the later selectivity comparison.

Usage:
    python models/psychiatric/hrh1_activity/fetch_chembl_data.py
"""
...
OUTPUT_CSV = ROOT / "datasets" / "raw" / "chembl_hrh1_ki_raw.csv"
OUTPUT_MANIFEST = ROOT / "datasets" / "raw" / "chembl_hrh1_ki_manifest.json"
...
def main() -> int:
    ...
    manifest = {
        ...
        "url": BASE_URL,
        "target_chembl_id": TARGET_CHEMBL_ID,
        "target_name": "Histamine H1 receptor (HRH1)",
        "standard_type": STANDARD_TYPE,
        ...
        "note": (
            ...
            "ChEMBL had for CHEMBL231 on the retrieval date, via full "
