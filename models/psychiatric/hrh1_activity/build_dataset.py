#!/usr/bin/env python3
"""Build the HRH1 binding-affinity training dataset from the raw ChEMBL pull.

Mirrors `models/psychiatric/drd2_activity/build_dataset.py` exactly --
same standardisation, salt-form merging by inchikey_full,
discordance-aware geometric-mean aggregation, and pKi transform (higher
= stronger binding). See that file's docstring for the full rationale;
not repeated here.

Usage:
    python models/psychiatric/hrh1_activity/build_dataset.py
"""
...
RAW_CSV = ROOT / "datasets" / "raw" / "chembl_hrh1_ki_raw.csv"
RAW_MANIFEST = ROOT / "datasets" / "raw" / "chembl_hrh1_ki_manifest.json"
OUTPUT_CSV = ROOT / "datasets" / "processed" / "hrh1_activity_dataset.csv"
OUTPUT_MANIFEST = ROOT / "datasets" / "processed" / "hrh1_activity_dataset_manifest.json"

DATASET_ID = "hrh1_activity"
DATASET_VERSION = "v1"
...
def main() -> int:
    ...
    manifest = {
        ...
        "endpoint": "HRH1 (histamine H1 receptor) binding affinity, continuous regression target",
        "endpoint_definition": (
            "pki = 9 - log10(aggregated Ki in nM), i.e. -log10(Ki in molar). Higher pki means "
            "stronger predicted binding. Same convention as drd2_activity, so the two are on a "
            "directly comparable scale for selectivity work."
        ),
