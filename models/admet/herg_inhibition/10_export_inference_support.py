...
data needed at inference time, as one versioned artifact.

This is NOT a new computation -- it packages exactly the reference data
already validated in Phase 3 (reliability.py) and Phase 4
(06_uncertainty_calibration.py, 05_applicability_domain.py):

  * conformal nonconformity scores from the FROZEN calibration split
    (split_group 7), computed with the registered model -- never refit at
    inference time, which is what makes the coverage guarantee meaningful.
  * training fingerprints (groups 0-6) for the max-Tanimoto AD component.
  * training descriptors (groups 0-6) + a StandardScaler fit on them, for
    the k-NN-distance-in-descriptor-space AD component -- kept separate
    from the model's own input, which uses RAW (unscaled) descriptors
    concatenated with fingerprints (Random Forest does not need scaling;
    only the descriptor-space AD distance calculation does).
  * the k-NN distance threshold (95th percentile of training-internal
    distances) and the training scaffold set, for the scaffold-seen AD
    component.

Inference code loads this artifact once at startup and never recomputes
any of it -- recomputing would silently drift from what was actually
validated in Phase 3/4.

Usage:
    python models/admet/herg_inhibition/10_export_inference_support.py
"""
...
def main() -> int:
    ...
    # --- Conformal: nonconformity on the frozen calibration split ---
    ...
    # --- AD: descriptor-space scaler + k-NN threshold (fit on train only) ---
    ...
    manifest = {
        ...
        "note": (
            "Frozen at Phase 5 export time from the SAME calibration/train split used in Phase 3/4. "
            "Never recomputed at inference time. Regenerating this file requires re-running this "
            "script deliberately, and should only happen alongside a new model_version."
        ),
    }
