#!/usr/bin/env python3
"""Curate the hERG raw ChEMBL pull into a per-measurement ledger + curated view.

Reads the exact same raw CSV ``build_dataset.py`` reads
(``datasets/raw/chembl_herg_ic50_raw.csv``) independently, and writes a
new, additive artifact under ``datasets/curated/`` -- this script never
reads or writes anything under ``datasets/processed/``, and never touches
``build_dataset.py``, ``fetch_chembl_data.py``, or ``prepare_features.py``.
Nothing about the live model's training data changes as a result of
running this.

Unlike ``build_dataset.py``, which drops censored measurements, bad-
validity-flagged records, and discordant compounds before writing
anything, this script keeps every raw row visible with an explicit
``curation_status``/``exclusion_reason`` -- see
``docs/data-curation/rules.md`` for what each reason means and
``docs/data-curation/README.md`` for how this relates to the live pipeline.

The actual standardise -> ledger -> aggregate -> report sequence lives in
``drugsim_curation.pipeline.curate_raw_rows`` -- this script is thin
orchestration around that shared function (also used by the CYP3A4 driver
and the golden-fixture generator/test), not a second copy of the sequence.

Usage:
    python models/admet/herg_inhibition/curate_measurements.py
    python models/admet/herg_inhibition/curate_measurements.py --no-network
        (skip live ChEMBL assay-metadata fetch; use only the local cache)
"""
...
ASSAY_CACHE = ROOT / "datasets" / "reference" / "chembl_herg_assay_metadata_cache.json"
OUT_DIR = ROOT / "datasets" / "curated"
LEDGER_CSV = OUT_DIR / "herg_inhibition_measurements_ledger.csv"
CURATED_CSV = OUT_DIR / "herg_inhibition_curated_compounds.csv"
REPORT_JSON = OUT_DIR / "herg_inhibition_curation_report.json"
MANIFEST_JSON = OUT_DIR / "herg_inhibition_curation_manifest.json"

ENDPOINT = "herg_inhibition"
SOURCE_DATASET_ID = "chembl_herg_ic50_raw"
SOURCE_ID = "chembl"
TARGET_CHEMBL_ID = "CHEMBL240"

#: Mirrors build_dataset.py's own exclusion set for THIS endpoint exactly --
#: see that file. CYP3A4's driver script uses a different set deliberately;
#: see its own module docstring for why.
BAD_VALIDITY_FLAGS = frozenset({"Potential transcription error", "Potential author error"})

#: Same convention as build_dataset.py: 10 uM.
...
def main() -> int:
    """Run the curation pipeline for hERG and write all outputs."""
