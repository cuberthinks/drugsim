#!/usr/bin/env python3
"""Train the hERG inhibition classifier on Phase 11's curated dataset.

Phase 12 companion to `train.py`. The training PROCEDURE is copied
verbatim -- same three candidates, same hyperparameter grid, same random
seed, same train/validation groups -- so that any difference between this
model and the production one is attributable to the data source
(`datasets/curated/herg_inhibition_features_curated.npz` vs.
`datasets/processed/herg_inhibition_features.npz`), not to a different
training recipe. Writes to `experiments/curated_v1/`, never touching the
production `artifact/` directory.

Usage:
    python models/admet/herg_inhibition/train_curated.py
"""
...
FEATURES_NPZ = ROOT / "datasets" / "curated" / "herg_inhibition_features_curated.npz"
