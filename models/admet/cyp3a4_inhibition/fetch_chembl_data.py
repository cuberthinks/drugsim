#!/usr/bin/env python3
"""Fetch the complete real CYP3A4 (CHEMBL340) IC50 activity set from ChEMBL.

Standalone and reproducible, mirroring
``models/admet/herg_inhibition/fetch_chembl_data.py`` exactly (same
retrieval strategy, same pagination, same manifest shape) -- this script
changes only the target and the target-specific metadata, per Phase 9's
"reuse existing infrastructure" instruction.

Endpoint choice (docs/phase9/endpoint-selection.md): CYP3A4 (CHEMBL340,
"Cytochrome P450 3A4") was named in the hERG fetch script's own docstring
as the largest ChEMBL target considered and passed over during Phase 3's
endpoint selection (13,887 IC50 records at that time). Verified live again
on 2026-08-09 for Phase 9: still 13,887 total IC50 records, confirming the
target's bioactivity record has not shrunk and remains the largest
non-hERG single-target candidate checked.

Usage:
    python models/admet/cyp3a4_inhibition/fetch_chembl_data.py
"""
...
OUTPUT_CSV = ROOT / "datasets" / "raw" / "chembl_cyp3a4_ic50_raw.csv"
OUTPUT_MANIFEST = ROOT / "datasets" / "raw" / "chembl_cyp3a4_ic50_manifest.json"
...
def main() -> int:
    ...
    manifest = {
        ...
        "url": BASE_URL,
        "target_chembl_id": TARGET_CHEMBL_ID,
        "target_name": "Cytochrome P450 3A4 (CYP3A4)",
        "standard_type": STANDARD_TYPE,
        ...
        "note": (
            ...
            "ChEMBL had for CHEMBL340 on the retrieval date, via full "
