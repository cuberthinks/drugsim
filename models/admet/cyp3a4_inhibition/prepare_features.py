#!/usr/bin/env python3
"""Compute features and the global scaffold split for the CYP3A4 dataset.

Mirrors ``models/admet/herg_inhibition/prepare_features.py`` exactly --
same descriptor set, same Morgan fingerprint parameters, same scaffold-
level split-group assignment scheme (ADR-009). SPLIT_SALT is
endpoint-specific ("cyp3a4_inhibition_v1", not hERG's salt) so this
dataset gets its own independent split assignment, not a reuse of
hERG's.

Usage:
    python models/admet/cyp3a4_inhibition/prepare_features.py
"""
...
OUTPUT_NPZ = ROOT / "datasets" / "processed" / "cyp3a4_inhibition_features.npz"
OUTPUT_MANIFEST = ROOT / "datasets" / "processed" / "cyp3a4_inhibition_features_manifest.json"
