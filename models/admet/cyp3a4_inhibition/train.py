#!/usr/bin/env python3
"""Train the CYP3A4 inhibition baseline classifier.

Mirrors ``models/admet/herg_inhibition/train.py`` exactly -- classical
models only (Random Forest, Gradient Boosting, Logistic Regression),
small fixed hyperparameter grid selected on the validation group only.
Reports comparison against baselines.py's majority/descriptor-only
numbers so the champion model's selection is demonstrated, not merely
asserted (Phase 9 Sec 6/7).

Uses ONLY split groups 0-6 (train) and 8 (validation). Group 7
(calibration) and group 9 (test) are never touched by this script.

Usage:
    python models/admet/cyp3a4_inhibition/train.py
"""
...
BASELINES_REPORT = Path(__file__).resolve().parent / "baselines_report.json"
...
def main() -> int:
    ...
    baselines = json.loads(BASELINES_REPORT.read_text(encoding="utf-8"))["baselines"]
    champion_auc = round(float(roc_auc_score(y_val, val_probs)), 4)
    baseline_comparison = {
        "champion_roc_auc": champion_auc,
        "majority_class_balanced_accuracy": baselines["majority_class"]["balanced_accuracy"],
        "champion_balanced_accuracy": round(float(balanced_accuracy_score(y_val, val_preds)), 4),
        "descriptor_only_logistic_regression_roc_auc": baselines["descriptor_only_logistic_regression"]["roc_auc"],
        "descriptor_only_random_forest_roc_auc": baselines["descriptor_only_random_forest"]["roc_auc"],
        "improvement_over_best_baseline_roc_auc": round(
            champion_auc - max(
                baselines["descriptor_only_logistic_regression"]["roc_auc"],
                baselines["descriptor_only_random_forest"]["roc_auc"],
            ),
            4,
        ),
        "meaningfully_beats_baselines": champion_auc > max(
            baselines["descriptor_only_logistic_regression"]["roc_auc"],
            baselines["descriptor_only_random_forest"]["roc_auc"],
        ),
    }
    print(f"\nBaseline comparison: {json.dumps(baseline_comparison, indent=2)}", file=sys.stderr)
