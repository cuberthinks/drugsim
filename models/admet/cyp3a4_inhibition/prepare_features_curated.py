#!/usr/bin/env python3
"""Compute features and the global scaffold split for the CURATED CYP3A4 dataset.

Mirrors ``models/admet/herg_inhibition/prepare_features_curated.py`` --
see that file for the full rationale. Reads Phase 11's
`datasets/curated/cyp3a4_inhibition_curated_compounds.csv` instead of
`datasets/processed/cyp3a4_inhibition_dataset.csv`; never touches
`datasets/processed/`. SPLIT_SALT is endpoint-specific
("cyp3a4_inhibition_v1"), matching `prepare_features.py`'s own split.

Usage:
    python models/admet/cyp3a4_inhibition/prepare_features_curated.py
"""
...
CURATED_CSV = ROOT / "datasets" / "curated" / "cyp3a4_inhibition_curated_compounds.csv"
OUTPUT_NPZ = ROOT / "datasets" / "curated" / "cyp3a4_inhibition_features_curated.npz"
OUTPUT_MANIFEST = ROOT / "datasets" / "curated" / "cyp3a4_inhibition_features_curated_manifest.json"
PRODUCTION_MANIFEST = ROOT / "datasets" / "processed" / "cyp3a4_inhibition_features_manifest.json"

SPLIT_SALT = "cyp3a4_inhibition_v1"
N_SPLIT_GROUPS = 10
