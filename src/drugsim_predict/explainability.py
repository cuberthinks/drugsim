...
Incident (first deploy of this feature): cyp3a4_inhibition's explainer (500
trees, depth up to 131) measured a 280MB resident-memory jump in production
and crashed the process on the 512MB instance -- caught only after deploy.
EXPLAINABLE_MODEL_IDS below is the single source of truth for which
endpoints this feature is actually offered for; it is deliberately hERG-only
until CYP3A4's memory cost is brought down or the instance is upgraded.
tests/unit/test_predict_explainability_memory.py asserts every ID in this
set fits a real measured memory budget -- that test is the gate that must
pass before adding an ID back here, not a suggestion.
"""
...
#: The only endpoints POST /predict/explain will serve -- see this module's
#: docstring "Incident" note. Anything else raises UnexplainableEndpointError
#: with a clear, honest message rather than attempting the computation and
#: risking the same crash again.
EXPLAINABLE_MODEL_IDS = frozenset({"herg_inhibition"})


class UnexplainableEndpointError(Exception):
    """Raised for a real, registered, servable endpoint that nonetheless
    does not support /predict/explain yet -- a scoping limit, not a bug, an
    unknown endpoint, or an unavailable one (those already have their own
    errors)."""
