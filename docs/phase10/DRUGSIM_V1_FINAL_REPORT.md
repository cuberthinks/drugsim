...
- **Licensing (unresolved, critical): both live endpoints train directly on ChEMBL data (CC BY-SA 3.0).** Whether that ShareAlike copyleft reaches the trained model weights and predictions themselves ("Adapted Material") is a genuinely unsettled legal question — not a formality. The original risk register (`docs/tds/10-risk-register.md`, R1) named a written legal opinion as a hard gate required *before Phase 3 completes*; that opinion was never obtained, and no phase from 3 through 10 re-raised the gap. The product has shipped through v1.0 on exactly the data this risk describes, without it having been resolved or escalated. This is not an engineering task — it requires a specific legal opinion, not yet requested.
